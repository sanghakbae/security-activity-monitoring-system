...
export default function ExecutionPage({
  ...
  return (
    <div className="space-y-2" style={{ fontFamily: 'KoPubDotumMedium' }}>
      <div className="flex items-center gap-1.5 rounded-md border border-slate-200 bg-white px-1.5 py-1 shadow-sm">
        ...
        {paginatedExecutionRecords.map((item) => {
          ...
          return (
            <button
              ...
              className={`w-full rounded-md border px-1.5 py-1.5 text-left shadow-sm transition hover:shadow-md ${
              ...
              <div className="flex items-center gap-2">
                <div className="min-w-0 flex-1">
                  <div className="mb-0.5 flex items-center gap-1">
                    <span className="shrink-0 rounded-sm bg-slate-200 px-1.5 py-0.5 text-[11px] font-semibold leading-none text-slate-800">
                      {item.frequencyLabel}
                    </span>

                    <span
                      className={`shrink-0 rounded-sm px-1.5 py-0.5 text-[11px] font-semibold leading-none ${getStatusStyle(
                        item.status,
                      )}`}
                    >
                  ...
                  <div className="break-keep text-[14px] font-semibold leading-4.5 text-slate-900">
                    {item.title}
                  </div>
                </div>
                ...
                <div className="ml-auto shrink-0 text-right">
                  ...
                  <div className="mt-0.5 text-[14px] font-semibold leading-none text-slate-900">
                    {item.dueDate.slice(0, 7)}
                  </div>
                </div>

                {item.status === '완료' && (
                  <div className="flex shrink-0 items-center justify-center gap-1 rounded-md border border-emerald-200 bg-emerald-50 px-1 py-0.5 text-emerald-700">
                    <CheckCircle2 className="h-4 w-4" />
                    <span className="text-[12px] font-semibold">완료</span>
                  </div>
                )}
              </div>
            </button>
